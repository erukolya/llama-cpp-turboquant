...
int main() {
    ...
    std::string error;
    require(registry.add(make_source("blk.0.ffn_gate_exps.weight"), GGML_TYPE_F32,
        placement, &gate_cpu, &gate_gpu, error), error.c_str());
    ...
    require(!registry.add(make_source("blk.0.ffn_gate_exps.weight"), GGML_TYPE_F32,
        placement, &gate_cpu, &gate_gpu, error), "duplicate source must be rejected");
